...
export async function calculateRates(
  ...
  for (const carrier of activeCarriers) {
    ...
    results.push({
      ...
      peakSurchargeInr,
      usInboundInr,
      formalClearanceInr: (carrier === 'ups' && input.upsOptions?.formalClearance) ? UPS_FORMAL_CLEARANCE : 0,
      ddpInr: (carrier === 'ups' && input.upsOptions?.ddp) ? UPS_DDP : 0,
      signatureInr: (carrier === 'ups' && input.upsOptions?.signature) ? UPS_SIGNATURE : 0,
      remoteAreaInr: (carrier === 'ups' && input.upsOptions?.remoteArea) ? Math.max(UPS_REMOTE_PER_KG * effectiveChargeable, UPS_REMOTE_MIN) : 0,
      oversizeFeeInr: (carrier === 'ups' && girth > 400) ? UPS_OVERSIZE_FEE : 0,
      upsFixedInr,
